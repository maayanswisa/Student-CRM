...
export const studentSchema = z.object({
  ...
  mother_name: z.string().trim().min(1, "שם ההורה נדרש"),
  student_phone: z.string().trim().optional().or(z.literal("")),
  mother_phone: z.string().trim().min(1, "טלפון ההורה נדרש"),
  address: z.string().trim().min(1, "כתובת נדרשת"),
  grade: z.string().trim().min(1, "כיתה נדרשת"),
  academic_level: z.string().trim().min(1, "רמת לימוד נדרשת"),
  school: z.string().trim().min(1, "בית ספר נדרש"),
